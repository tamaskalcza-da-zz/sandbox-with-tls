...
function generate_key_and_signed_certificate() {
  ...
  openssl req -newkey rsa:$bit_length -nodes -keyout "$key" -out "$request" -subj "/O=Generator Script"
...
openssl req -newkey rsa:$bit_length -nodes -keyout "$ca_key" -x509 -days 365 -out "$ca_certificate" -subj "/O=Generator Script"
...
generate_key_and_signed_certificate $server_key $server_certificate $ca_key $ca_certificate
...
generate_key_and_signed_certificate $client_key $client_certificate $ca_key $ca_certificate
